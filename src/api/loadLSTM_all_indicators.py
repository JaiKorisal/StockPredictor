...
def predict_lstm(symbol):
    ...
    for y in result:
        if y.columns[0] == 'ATR':
            ab = pd.DataFrame(y['Predictions'] + nextPrice[-1])
            ab.to_csv("test.csv")
            var = float(y['Predictions'].iloc[0] + nextPrice[-1])

    print("Predicted Opening Price LSTM {:0.2f}".format(var))

    prediction_output_lstm = {
        "Predicted_Opening_Price_LSTM": var
    }

    return prediction_output_lstm
